    ...
    <div className="max-w-3xl mx-auto px-6 py-12">
      <div className="card p-10 rounded-3xl">
        <div className="flex items-center gap-6 mb-10">
          <div className="w-20 h-20 rounded-full bg-[var(--accent)] flex items-center justify-center text-[oklch(99%_0.005_70)] text-4xl font-bold">
            {user.name?.[0] || 'م'}
          </div>
          <div className="flex-1">
            ...
            {user.role === 'admin' && (
              <span className="inline-flex items-center gap-1.5 mt-2 px-3 py-1 rounded-full bg-[var(--accent)]/20 text-[var(--accent)] text-xs font-semibold">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="p-6 bg-[var(--secondary)] rounded-2xl">
            <h3 className="font-semibold mb-3 flex items-center gap-2 text-sm">
              <Crown className="w-4 h-4 text-[var(--accent)]" /> اشتراكي
            </h3>
            {subscription !== 'none' ? (
              <>
              ...
              <>
                <p className="text-lg font-bold text-[var(--muted-foreground)]">لا يوجد اشتراك</p>
                <Link href="/subscriptions" className="text-xs text-[var(--accent)] underline">ابدأ الآن</Link>
              </>
            )}
          </div>
          ...
          <div className="p-6 bg-[var(--secondary)] rounded-2xl">
            <h3 className="font-semibold mb-3 flex items-center gap-2 text-sm">
              <ShoppingBag className="w-4 h-4 text-[var(--accent)]" /> الوصفات المشتراة
            </h3>
          ...
          <div className="p-6 bg-[var(--secondary)] rounded-2xl">
            <h3 className="font-semibold mb-3 flex items-center gap-2 text-sm">
              <Heart className="w-4 h-4 text-[var(--accent)]" /> المفضلة
            </h3>
            <p className="text-4xl font-bold">{favorites.length}</p>
          </div>
        </div>

        <div className="mt-8 pt-8 border-t border-[var(--border)] flex flex-wrap gap-4">
